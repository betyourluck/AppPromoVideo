...
**接地の限界**: live 未実施。**rev9 より前に作った run は `base/` を持たないので焼き直せない** —
`reburn_caption` はその旨をエラーで返す (移行はしない。作り直せば済むため)。
ディスクは run あたり約 2 倍になる (6 シーンで 18 MB → 36 MB)。古い run の掃除機構は無い。
