...
(→ 2026-09-14 ユーザーが配布ビルドで保存を確認。こちらで run `20260914-071122` を読み、promo.json と scenes.md の更新時刻が同じ秒で、5 シーンの motion_prompt / video_prompt がすべて scenes.md に載っていることを確かめた)。
...
**接地の限界**: **GUI 未目視** (→ 2026-09-14 ユーザーが配布ビルドで並び替え・複製・削除を確認)。実ファイルでの付け替えは backend のテストで固定したが、
...
**接地の限界**: **GUI 未目視。** スポットライトの位置・カードの置き場・遷移は実機で見ていない (→ 2026-09-14 ユーザーが配布ビルドで 4 歩を確認。窓の最大化・縮小のどちらでも位置に問題なし、ライトテーマでも問題なし)。
...
**接地の限界**: 測ったのは**ブラウザ**で、Tauri の WebView では未目視 (→ 2026-09-14 ユーザーが配布ビルドでタイトルバーと案内の標を見比べて確認)。窓は 1280x720 で、
...
**接地の限界**: **配布ビルドでは未確認** (→ 2026-09-14 ユーザーが配布ビルドで確認: 右クリック・F5 / Ctrl+R・ラベルの選択が効かず、入力欄・プロンプト本文・エラー文・進捗ログは選べる)。dev サーバーでは印が立たないので、規則の効き目だけを
...
**ユーザー目視 OK (2026-09-13 23:46、Tauri の実画面 1920 幅・画像 off)**: 回る矢印 / 3 段 (解析が active) / 経過 0:08 / 最新の進捗 1 行が中央に出ている。
**接地の限界**: 配布ビルド (installer から入れたもの) に乗るのは次のタグから。

## rev50 (2026-09-14、費用の chip に「LLM」の印)

**ユーザーの疑問** (2026-09-13)「画像を生成していないのに費用?」。chip は `1.674 USD` とだけ出ていて、**何の費用かを書いていなかった**。
中身は解析 + 構成の LLM 費用で (契約 `RunStats.cost_usd` / `RunRecord.cost_usd` = analyze + plan の合計)、参照画像の生成は**一度も数えていない**。

191. **chip が名乗る** — `LLM {cost} USD` (`scene.costChip`、3 言語とも同じ文字)。ホバーは「解析と構成で LLM にかかった費用
     (参照画像の生成は含まない)」(`scene.costTitle`) で、実行直後はその下の行に段ごとの経過時間 (`scene.durationsTitle`) を添える。
     以前の title は経過時間だけで、**履歴から開いた run では空**だった。文字と title は `runHeaderStats` (`runs.ts`) が組み立てる —
     `ScenePanel` は並べるだけ (数値の出所と同じ関数に説明も置く。rev36 の「正本から出す」と同じ場所)。
     **出すか出さないかは変えない** — 記録が無ければ chip ごと出さない (rev42)。

**PoC**: `runs.test.ts` に 1 本 (Red = `costLabel` が undefined → Green)。vitest 127 → 128、build green。
**実測** (ブラウザのペイン、ユーザーの vite に接続、今回の変更がページに届いていることを `scene.costChip` の有無で先に確認):
ストアにダミーの run を注入 → chip の文字 `LLM 1.674 USD`、title `解析と構成で LLM にかかった費用 (参照画像の生成は含まない)\n解析 12000 ms / 構成 30000 ms`。
費用 null の run に差し替えると chip は `15s · 16:9` と `構成 1 回目で通過` の 2 つだけ (費用の chip は出ない)。
**幅**: 印の 4 字で chip は 131px。1280 幅 (見出しの列 298px) は chip が 2 段に並ぶだけで各 32px (中で折れない)、1920 幅 (380px) は 1 段にちょうど収まる。
ブラウザのペイン (800px) では chip の中で単語ごとに折れたが、構成の chip も同じく折れており、この幅は実際の窓より狭い。
**触っていないもの**: 履歴の表の列見出し「費用」(`runs.colCost`) も中身は同じ LLM 費用だが、ユーザーの依頼は chip なので据え置き。
**接地の限界**: Tauri の実画面は未目視。配布物に乗るのは次のタグから (→ v0.1.2 に乗った。2026-09-14 ユーザーが配布ビルドで費用の表示とホバーを確認)。
...
**接地の限界**: Tauri の実画面は未目視。agy の run で進捗ログの行が変わったこと (194) は live で見ていない (テストのみ) (→ 2026-09-14 ユーザーが配布ビルドで設定画面と実 run の進捗ログを確認)。
...
- [ ] rev37 (2026-09-12): プロンプトの書き換え (145〜148、鉛筆で編集モード)。crates 162 / backend 17 / vitest 75 / build green。**Tauri の GUI は未目視 (GUI からの保存は未実施)** (→ 2026-09-14 ユーザーが配布ビルドで保存を確認。promo.json と scenes.md が同じ秒に書き直され、全シーンの 2 本とも scenes.md に一致)
...
- [x] rev43 (2026-09-12): シーンの並び替え / 複製 / 削除 (169〜173、契約 `SceneEdit`)。crates 188 / backend 25 / vitest 90 / build green。**ユーザーが配布ビルドで目視確認 (2026-09-14)**
- [x] rev44 (2026-09-12): 初回起動のナビゲーション (174〜178、契約 `FirstRunTour`。Lorekeel からの移植)。vitest 110 / build green。**ユーザーが配布ビルドで確認 (2026-09-14)** — 4 歩とも、窓を最大化・縮小しても位置に問題なし、ライトテーマでも問題なし
- [x] rev45 (2026-09-12): 案内をもう一度見る口 (179)。初回判定が既存ユーザーを弾くので入口が無かった (実機で発覚)。vitest 112 / build green。**ユーザーが配布ビルドで目視確認 (2026-09-14)**
- [x] rev46 (2026-09-12): ブランドの標を 1 か所に (180)。ブラウザで実効値を突合、1 歩目の枠とカードも実測。vitest 112 / build green。**ユーザーが配布ビルドで目視確認 (2026-09-14)**
- [x] rev47 (2026-09-12): 配布ビルドの締め (181〜183、契約 `DesktopGuards`)。右クリックと F5 は既にあり、文字の選択を追加。vitest 118 / build green。**ユーザーが配布ビルドで確認 (2026-09-14)** — 右クリック・F5 / Ctrl+R・ラベルの選択が効かず、入力欄・プロンプト本文・エラー文・進捗ログは選べる
...
- [x] rev50 (2026-09-14): 費用の chip に「LLM」の印 (191)。`LLM … USD` + 画像生成を含まない旨のホバー。vitest 128 / build green、ブラウザで実測。**ユーザーが配布ビルドで目視確認 (2026-09-14)**
